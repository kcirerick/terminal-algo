...
class GameState:
    ...
    def __init__(self, config, serialized_string):
        ...
        global WALL, SUPPORT, TURRET, SCOUT, DEMOLISHER, INTERCEPTOR, REMOVE, UPGRADE, STRUCTURE_TYPES, ALL_UNITS, UNIT_TYPE_TO_INDEX, STRING_TO_VARIABLE
        UNIT_TYPE_TO_INDEX = {}
        ...
        self.action_space = create_action_space(self.ARENA_SIZE)
    ...
    def _is_valid_remove(self, move):
        if move[2][1] < self.HALF_ARENA and self.contains_stationary_unit(move[2]):
            return True
        return False
  
    def _have_enough_structure_points(self, unit):
        return self.get_resource(0, 0) > self.type_cost(unit.unit_type, True)

    def _is_valid_upgrade(self, move):
        if move[2][1] < self.HALF_ARENA and self.contains_stationary_unit(move[2]):
            for unit in self.game_map[move[2][0], move[2][1]]:
                if unit.stationary and (not unit.upgraded) and self.have_enough_structure_points(unit):
                    return True
        return False
    ...
    def is_valid_action(self, move, game_state):
        action = move[0]
        if (action == "SPAWN"):
            return self.can_spawn(STRING_TO_VARIABLE[move[1]], move[2]) # SHOULD CHANGE 'move[1]' to the actual unit_type when implemented
        else if (action == "REMOVE"):
            return self.is_valid_remove(move)
        else if(action == "UPGRADE")
            return self.is_valid_upgrade(move)
        return False